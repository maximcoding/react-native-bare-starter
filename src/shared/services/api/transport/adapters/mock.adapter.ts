 ...
 *
 * FEATURES:
 *   - Implements OPS.AUTH_LOGIN / OPS.AUTH_REFRESH / OPS.AUTH_SESSION
 *   - Implements OPS.USER_ME / OPS.USER_PROFILE / OPS.USER_UPDATE_PROFILE
...
export const mockAdapter: Transport = {
  async query<TResponse = unknown, TVariables = unknown>(
    ...
    switch (operation) {
      ...
      case OPS.USER_PROFILE: {
        requireAuth()
        const userId = (variables as { userId?: string } | undefined)?.userId
        if (userId && userId !== dbUser.id) {
          const err: any = new Error('Mock: user not found')
