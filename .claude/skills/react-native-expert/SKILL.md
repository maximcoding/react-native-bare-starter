...
description: Builds, optimizes, and debugs cross-platform mobile applications with React Native (bare workflow). Implements navigation hierarchies (tabs, stacks, drawers), configures native modules, optimizes FlatList/FlashList rendering with memo and useCallback, and handles platform-specific code for iOS and Android. Use when building a bare React Native app, setting up navigation, integrating native modules, improving scroll performance, handling SafeArea or keyboard input.
license: MIT
metadata:
  author: https://github.com/Jeffallan
  version: "1.1.0"
  domain: frontend
  triggers: React Native, bare workflow, mobile app, iOS, Android, cross-platform, native module
  role: specialist
...
Senior mobile engineer building production-ready cross-platform applications with React Native.

## Core Workflow

1. **Setup** — React Navigation (native stack + bottom tabs), TypeScript config → _run `npx react-native doctor` to verify environment; fix any reported issues before proceeding_
...
### Error Recovery
- **Metro bundler errors** → clear cache with `npm start` (already includes `--reset-cache`), then restart
- **iOS build fails** → check Xcode logs → resolve native dependency or provisioning issue → run `npx pod-install ios` then `npm run ios`
- **Android build fails** → check `adb logcat` or Gradle output → resolve SDK/NDK version mismatch → `npm run android:rebuild`
- **Native module not found** → `npm install <module>`, then `npx pod-install ios` for iOS native rebuild
...
|-------|-----------|-----------|
| Navigation | `references/react-navigation.md` | React Navigation, tabs, stacks, deep linking |
...
React Native 0.82+ (bare workflow), React Navigation 7, Reanimated 4, Gesture Handler, MMKV, TanStack React Query 5, Zustand 5, FlashList, react-native-bootsplash
