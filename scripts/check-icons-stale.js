...
function parseIconsTsNames(src) {
  // match import lines from @assets/svgs/*.svg
  const importRe =
    /import\s+([A-Za-z0-9_]+)\s+from\s+['"]@assets\/svgs\/([^'"]+\.svg)['"];?/g
  const names = []
  let m
  // biome-ignore lint/suspicious/noAssignInExpressions: common regex loop idiom
...
if (missingInIcons.length || extraInIcons.length) {
  ...
  if (extraInIcons.length) {
    console.error(
      '  Import exists but file missing:\n   - ' + extraInIcons.join('\n   - '),
    )
  }
