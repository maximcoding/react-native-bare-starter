...
9. [User Interface](#9-user-interface) — **MEDIUM**
   ...
   - 9.5 [Use Optimized Image for Lists](#95-use-optimized-image-for-lists)
...
such as `react-native-fast-image` or a similar caching library.
...
**For stacks:** Use `@react-navigation/native-stack` or file-based router's default

stack (which uses native-stack). Avoid `@react-navigation/stack`.

**For tabs:** Use `react-native-bottom-tabs` (native) or file-based router's native
...
- [React Native Bottom Tabs with File-based routing](https://oss.callstack.com/react-native-bottom-tabs/docs/guides/usage-with-file-based router)

- [File-based routing Native Tabs](https://reactnative.dev/router/advanced/native-tabs)
...
**Correct: file-based router uses native stack by default**

```tsx
// app/_layout.tsx
import { Stack } from 'file-based router'
...
**Correct: file-based router native tabs**

```tsx
// app/(tabs)/_layout.tsx
import { NativeTabs } from 'file-based router/unstable-native-tabs'
...
```tsx
import { useRouter } from 'file-based router'
...
### 9.5 Use Optimized Image for Lists

**Impact: HIGH (memory efficiency, caching, blurhash placeholders, progressive loading)**

Use a caching image component (e.g. `react-native-fast-image`) instead of React Native's `Image`. It provides memory-efficient caching, blurhash placeholders, progressive loading, and better performance for lists.
...
For cross-platform (web + native), use `SolitoImage` from `solito/image` or a similar abstraction.

Reference: [https://reactnative.dev/versions/latest/sdk/image/](https://reactnative.dev/versions/latest/sdk/image/)
...
zoom, and pan-to-close. Works with any image component including `react-native-fast-image`.
...
Use the `font config plugin` config plugin to embed fonts at build time instead of

`useFonts` or `Font.loadAsync`. Embedded fonts are more efficient.

[Font documentation](https://reactnative.dev/versions/latest/sdk/font/)

**Incorrect: async font loading**

```tsx
import { useFonts } from 'font config plugin'
...
After adding fonts to the config plugin, run `npx react-native run-ios` and rebuild the
...
5. [https://reactnative.dev](https://reactnative.dev)
